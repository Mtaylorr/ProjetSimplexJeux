...
function readInputFile(inputFile::String)
    ...
    close(datafile)
    sz = Int64(length(data[1]))
    res = Array{Int64}(undef, 0,Int64((sz+1)/2))
    # For each line of the input file
    for line in data
        ligne =  Array{Int64}(undef, 1,0)
        t=split(line,",")
        for val in t
            intval = parse(Int64, val)
            ligne = hcat(ligne,[intval])
        end
        res=vcat(res,ligne)
    end
    return res
...
function displayGrid(inputFile::String)
 # Open the input file
    datafile = open(inputFile)
...
function displaySolution(inputFile::String)
 # Open the input file
    datafile = open(inputFile)
